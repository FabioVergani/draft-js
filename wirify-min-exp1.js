...
function wfClosure(M) {
    ...
    function f() {
        var ag = M(document).width(),
            ak = M(document).height(),
            af = M("#wf-info").wfWidth(),
            ah = M("#wf-info").wfHeight(),
            aj = parseInt(N * af);
    ...
    function R() {
        ...
        var ag = Math.max(0, g) + Math.min(n, 1000) - 50 - af;
    ...
    function C() {
        ...
        u();
        S ? M("#wirify").remove() : M("#wirify").hide()
    }
